...
class Errors:
    def __init__(self, error, y_test, window_size):
        """
        ...
        # self.e_s[:l_s] = \
        #         [np.mean(self.e_s[:l_s * 2])] * l_s
        # print(f'early + smoothed prediction error:{self.e_s}')
        # print(f'early + smoothed prediction error shape:{self.e_s.shape}')

        self.normalized = np.mean(self.e / np.ptp(y_test))

    def append_new_errors(self, new_y_hat, new_y_test):
        prev_length = self.y_test.shape[0]
        self.e = np.append(self.e, [np.mean(abs(y_h-y_t)) for y_h, y_t in
                  zip(new_y_hat, new_y_test)], axis = 0)
        self.y_test = np.append(self.y_test, new_y_test, axis = 0)
        self.y_hat = np.append(self.y_hat, new_y_hat, axis = 0)

        smoothing_window = int(batch_size * window_size
                               * smoothing_perc)
        if not len(new_y_hat) == len(new_y_test):
            raise ValueError('len(y_hat) != len(y_test): {}, {}'
                             .format(len(new_y_hat), len(new_y_test)))

        # smoothed prediction error
        self.e_s = pd.DataFrame(self.e).ewm(span=smoothing_window)\
            .mean().values.flatten()

        self.normalized = np.mean(self.e / np.ptp(self.y_test))
        self.window_size = window_size
        self.n_windows = int((self.y_test.shape[0] -
                              (batch_size * self.window_size))
                             / batch_size)
        return prev_length

    def remove_prev_value(self):
        if math.floor(self.n_windows) > 2:
          reduction_index = len(self.y_test) - 1024
          self.y_test = self.y_test[-1024:]
          self.y_hat = self.y_hat[-1024:]
          self.i_anom =  self.i_anom - reduction_index #np.array([])
          self.i_anom = self.i_anom[self.i_anom >= 0]
          self.E_seq = [(e_seq[0] - reduction_index, e_seq[1] - reduction_index) for e_seq in self.E_seq if e_seq[0] - reduction_index >= 0]
          self.E_point = [p-reduction_index  for p in self.E_point if p - reduction_index>=0]
          self.anom_scores = [score for score in self.anom_scores if score['start_idx'] - reduction_index >= 0]
          for score in self.anom_scores:
            score['start_idx'] -= reduction_index
            score['end_idx'] -= reduction_index
    ...
    def process_batches(self):
        ...
        """
        if self.n_windows < 0:
          self.adjust_window_size(self.y_test)

        idx = (window_size * batch_size) + (self.n_windows * batch_size)
        if idx > self.y_test.shape[0]:
          self.n_windows+=1

        for i in range(0, self.n_windows+1):
            # pdb.set_trace()
            prior_idx = i * batch_size
            idx = (window_size * batch_size) \
                + (i * batch_size)
            if i == self.n_windows:
                idx = self.y_test.shape[0]
            window = ErrorWindow(self.y_test, prior_idx, idx, self, i)
        ...
        if len(self.i_anom) > 0:
            ...
            self.E_seq = [(int(g[0]), int(g[-1])) for g in groups
                          if not g[0] == g[-1]]
            self.E_point = [g[0] for g in groups if len(np.unique(g)) == 1]
            ...
            # untuk sementara tidak perlu + l_s karena bridge realtime tinggal dikurang data yang baru masuk

            # self.E_seq = [(e_seq[0] + l_s,
            #                e_seq[1] + l_s) for e_seq in self.E_seq]

            self.merge_scores()


class ErrorWindow:
    def __init__(self, y_test, start_idx, end_idx, errors, window_num):
        """
    ...
    def compare_to_epsilon(self, errors_all, inverse=False):
        ...
        if not (self.sd_e_s > (.05 * self.sd_values) or max(self.e_s)
            ...
            print("Check: scale of errors compared to values too small?")
            return
        ...
        batch_position = self.window_num * batch_size
